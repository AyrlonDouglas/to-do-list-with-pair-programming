...
import { IEditItemInToDoList } from '@/core/domain/usecases/IEditItemInToDoList'
...
export class EditItemInToDoList implements IEditItemInToDoList {
	...
	edit(): void {
		const itemToEdit = this.list.items.find(
			(itemInList) => itemInList.id.value === this.item.id.value,
		)
		if (itemToEdit) {
